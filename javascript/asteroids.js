	...
	for (let i = 0; i < rockCnt; i++) {
		...
		asteroids.push(
			new Asteroid(
				...
				getSafeRandomFloat(0, yLimit - 150),
				getRandomFloat(-Math.abs(rock_max_v), rock_max_v),
				getRandomFloat(-Math.abs(rock_max_v), rock_max_v),
				Asteroid.colors[Math.floor(getRandomFloat(0, 5))],
				'generic',
				false,
				20
			)
...
function makeRock(id, w, h, color, scale, path) {
	let newRock = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
	...
	let rockPath = document.createElementNS(
	...
	for (let i = 0; i < cnt; i++) {
		...
		asteroids.push(
			new Asteroid(
				...
				x,
				y,
				getRandomFloat(-Math.abs(maxVel), maxVel),
				getRandomFloat(-Math.abs(maxVel), maxVel),
				Asteroid.colors[Math.floor(getRandomFloat(0, 5))],
				'generic',
				false,
		...
		makeRock(
			asteroids[asteroids.length - 1].id,
			asteroids[asteroids.length - 1].width,
			asteroids[asteroids.length - 1].height,
			asteroids[asteroids.length - 1].color,
			scale,
			Asteroid.rocksLrg[Math.floor(getRandomFloat(0, 3))]
		);
	}
